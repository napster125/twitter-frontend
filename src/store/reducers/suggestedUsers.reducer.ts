...
const suggestedUsersReducer = (state: suggestedUsersState = initialState, action: suggestedUsersAction): suggestedUsersState => {
    switch (action.type) {
        ...
        case suggestedUsersActionTypes.SET_SUGGESTED_USER:
            return {
                ...state,
                suggestedUsers: state.suggestedUsers.map((user: any) => user._id === action.payload._id ? action.payload : user),
            };
        default:
            return state;
    }
